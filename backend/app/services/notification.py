...
class EmailService:
    ...
    def send_exit_alert(cls, recipients: List[str], exits: List[Dict]):
        ...
        <head>
            <style>
                ...
                .status-tag {{ padding: 3px 8px; border-radius: 4px; font-size: 0.85em; font-weight: bold; }}
                .auto-closed {{ background-color: #d4edda; color: #155724; }}
                .manual-required {{ background-color: #f8d7da; color: #721c24; }}
                .alert-only {{ background-color: #e2e3e5; color: #383d41; }}
            </style>
        </head>
        <body>
            <h2>Trade Exit Notifications</h2>
            <p>The following positions have been closed or flagged for exit:</p>
            <table>
                <tr>
                    <th>Symbol</th>
                    <th>Side</th>
        ...
        for e in exits:
            # Result formatting
            pnl = e.get('pnl', 0.0)
            res_class = "profit" if pnl > 0 else ("loss" if pnl < 0 else "neutral")
            res_text = "PROFIT" if pnl > 0 else ("LOSS" if pnl < 0 else "EVEN")
            
            # R:R formatting
            rr = e.get('actual_rr') or e.get('rr_achieved', 0.0)
            
            # Status formatting
            if e.get('auto_closed'):
                status_html = '<span class="status-tag auto-closed">✅ Auto-Closed</span>'
            elif e.get('requires_manual_close'):
                status_html = '<span class="status-tag manual-required">⚠️ Manual Required</span>'
            else:
                status_html = '<span class="status-tag alert-only">📧 Alert Only</span>'
            
            symbol = e.get('symbol', '')
            precision = 2 if "JPY" in symbol else 4
            entry_price = e.get('price', 0.0)
            ...
            html_body += f"""
                <tr>
                    <td><b>{symbol}</b></td>
                    <td>{e['signal']}</td>
                    <td>{entry_price:.{precision}f}</td>
                    <td>{exit_price:.{precision}f}</td>
        ...
        html_body += f"""
            ...
            <p><small>Generated at {datetime.now().strftime("%H:%M %d/%m/%Y")}</small></p>
    ...
    def send_signal_alert(cls, recipients: List[str], signals: List[Dict]):
        ...
        for s in signals:
            ...
            sl_str = f"{sl:.{precision}f}" if isinstance(sl, (float, int)) else "N/A"
            tp_str = f"{tp:.{precision}f}" if isinstance(tp, (float, int)) else "Open"
        ...
        html_body += f"""
            ...
            <p><small>Generated by ASX/Forex Screener at {datetime.now().strftime("%Y-%m-%d %H:%M:%S")}</small></p>
